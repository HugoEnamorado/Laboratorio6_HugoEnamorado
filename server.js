...
app.get('/setup', (req, res) => {
    ...
    nick.save(function(err) {
        ...
        console.log('El usuario se ha guardado exitosamente');
...
apiRoutes.get('/', (req, res) => {
    res.json({ message: 'Bienvenido al api de programacion.com.py :D' });
});
...
apiRoutes.post('/authenticate', (req, res) => {
    ...
    User.findOne({
        ...
        if (!user) {
            res.json({ success: false, message: 'Authentication failed. Usuario no encontrado.' });
        } else if (user) {

            if (user.password != req.body.password) {
                res.json({ success: false, message: 'Authentication failed. Contraseña incorrecta.' });
            } else {
